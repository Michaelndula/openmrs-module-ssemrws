...
import org.openmrs.module.ssemrws.CoreUtils;
import org.openmrs.module.ssemrws.FormConfiguration;
import org.openmrs.module.ssemrws.calculation.CalculationUtils;
import org.openmrs.module.ssemrws.program.ProgramDescriptor;
import org.openmrs.module.ssemrws.program.ProgramManager;
...
public class FormManager implements ContentManager {
	...
	private void refreshTagHandlers() {
		for (TagHandler tagHandler : Context.getRegisteredComponents(TagHandler.class)) {
			...
			if (className.endsWith(tagHandlerClassSuffix)) {
				...
				log.warn(
				    "Not registering tag handler class " + className + ". Name does not end with " + tagHandlerClassSuffix);
			}
		}
	}
	...
	/**
	 * Filters the given collection of enabled forms to those applicable for the given application and
	 * patient
	 * 
	...
	protected List<FormDescriptor> filterForms(Collection<FormDescriptor> descriptors, AppDescriptor app, Patient patient) {
		...
		for (FormDescriptor descriptor : descriptors) {
			...
			if (patient.getGender() != null) {
				if (patient.getGender().equals("F") && descriptor.getGender() == FormDescriptor.Gender.MALE)
					continue;
				else if (patient.getGender().equals("M") && descriptor.getGender() == FormDescriptor.Gender.FEMALE)
					continue;
			}
